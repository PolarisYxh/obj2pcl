    ...
    , m_selectedPoint(-1)
{
	...
	verts->push_back(osg::Vec3f(0.5, 1., 0.0));
	verts->push_back(osg::Vec3f(0., 1., 0.0));
	verts->push_back(osg::Vec3f(-0.5, 1., 0.0));
    verts->push_back(osg::Vec3f(-1.f, 1.f, 0.f));
	verts->push_back(osg::Vec3f(-1.f, 0.5f, 0.f));
	verts->push_back(osg::Vec3f(-1.f, 0.f, 0.f));
	verts->push_back(osg::Vec3f(-1.f, -0.5f, 0.f));
    verts->push_back(osg::Vec3f(-1.f, -1.f, 0.f));
	verts->push_back(osg::Vec3f(-0.5f, -1.f, 0.f));
	verts->push_back(osg::Vec3f(0.f, -1.f, 0.f));
	verts->push_back(osg::Vec3f(0.5f, -1.f, 0.f));
    verts->push_back(osg::Vec3f(1.f, -1.f, 0.f));
	verts->push_back(osg::Vec3f(1.f, -0.5f, 0.f));
	verts->push_back(osg::Vec3f(1.f, 0.f, 0.f));
    ...
    osg::Vec4Array* clrWire = new osg::Vec4Array(16);//边数等于点数
    ...
    osg::Vec4Array* clrPts = new osg::Vec4Array(16);
...
{
    if (m_selectedPoint < 0 || m_selectedPoint>15) return;
...
osg::Vec3d DraggableWire::pick(int index)
{
    if (index<0 || index > 15) return osg::Vec3d(0,0,0);
    this->select();
    for (int i=0; i<16; ++i){
	...
	osg::Vec3Array* vertsPoints = dynamic_cast<osg::Vec3Array*> (m_points->getVertexArray());
	return (*vertsPoints)[index];
}
...
{
	for (int i = 0; i < 16; ++i) {
		this->setColorPoint(i, CLR_POINTS);
	}
