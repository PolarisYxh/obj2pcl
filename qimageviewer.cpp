...
int QImageViewer::openPCLFile(const QString& caption,
	const QString& dir,
	const QString& filer)
{
	initImageResource();
	return loadPCLResource(caption, dir, filer);
}
...
int QImageViewer::last(int flag)
{
    if (index < 0) {
        return -1;
    }
	...
	{
		return upgradePCLFileInfo(filename);
	}
}

int QImageViewer::next(int flag)
{
    if (index < 0) {
        return -1;
    }
...
int QImageViewer::loadPCLResource(const QString& caption,
	const QString& directory,
	const QString& filer)
{
	filename = QFileDialog::getOpenFileName(this, caption, directory, filer);
...
int QImageViewer::upgradePCLFileInfo(QString& filename)
{
	if (filename.isEmpty()) {
		return -1;
	}
...
int QImageViewer::getPCLFileInfoList(void)
{
	QFileInfo info;
	QFileInfoList infoList;
