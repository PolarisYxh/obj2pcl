...
osgUtil::LineSegmentIntersector::Intersection pointintersect;
...
void EventHandler::doIdleMouse(const osgGA::GUIEventAdapter& ea, osgGA::GUIActionAdapter& aa)
{
	if (ea.getEventType() == osgGA::GUIEventAdapter::PUSH && ea.getButton() == ea.LEFT_MOUSE_BUTTON)
	{
		...
		std::tie(isModeSame, intersection) = this->setMouseState<PointIntersector::Intersection, PointIntersector>(ea, aa, MOUSE_IDLE);
		if (m_mode == MOUSE_HOVER_POINT)
		{
			PointIntersector::Intersection intersectionPoint;
			std::tie(isModeSame, intersectionPoint) =
				this->setMouseState<PointIntersector::Intersection, PointIntersector>(ea, aa, MOUSE_IDLE);
			...
			pointindex = this->getSelectedPoint(intersection);
			pointintersect = intersection;
			//=intersection.getWorldIntersectPoint();//获得所选点的世界坐标
			//osg::Vec3d localpos = intersection.getLocalIntersectPoint();//获得原来坐标，如1，0，0
			//osg::Vec3 point3 = ckq1->getBound().center() * osg::computeLocalToWorld(ckq1->getParentalNodePaths()[0]);
			if (!(m_mode & EDIT_MODE::DRAG_MASK))
				osg::Vec3 tem=m_selection->pick(pointindex);
		}
		else
		{
			if (m_selection.get()) {
...
{
    if (!( (ea.getEventType() == osgGA::GUIEventAdapter::PUSH && ea.getButton() == ea.LEFT_MOUSE_BUTTON)//ea.getButtonMask()== osgGA::GUIEventAdapter::LEFT_MOUSE_BUTTON
           || (ea.getEventType() == osgGA::GUIEventAdapter::DRAG && ea.getButtonMask()== osgGA::GUIEventAdapter::LEFT_MOUSE_BUTTON)
